...
* **memory.fill**\(dst: `usize`, c: `u8`, n: `usize`\): `void` Fills `n` bytes at `dst` with the given byte value `c`. Emits the respective instruction if bulk-memory is enabled, otherwise ships a polyfill.
