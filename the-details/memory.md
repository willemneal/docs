...
The most prominent shortcoming of this layout alone is that accessing the values of an array must go through one level of indirection. For example, an `arr[i]` must first obtain a reference to the backing buffer and only then is able to read the actual value. One possible way to solve this is to make use of the [Multi-value proposal for WebAssembly](https://github.com/WebAssembly/multi-value) eventually, lowering typed arrays for example to multiple values that are passed around directly, so the reference to the backing buffer is already on the execution stack. Another is to provide more knowledge about specific structures to Binaryen, so it can cache the reference to backing buffer as long as the array does not grow, but it is unclear how feasible this is.

Another issue is that due to our reliance on Binaryen, which already provides all the utility to do optimizations, and our unwillingness to duplicate its code, it is still to decide how to get rid of unnecessary range checks. For now, there is the famous `unchecked(expression)` built-in that explicitly request no bounds checks on indexed access, but this certainly isn't forever.
